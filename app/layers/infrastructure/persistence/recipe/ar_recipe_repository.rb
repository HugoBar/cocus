      ...
      class ArRecipeRepository < Domains::Recipe::RecipeRepository
        ...
        def create(attributes)
          ...
          ActiveRecord::Base.transaction do
            # Collect all product IDs from the recipe's ingredients
            product_ids = domain_recipe.ingredients.map { |i| i.product_id }
            # Fetch all products from the database and index by id
            ...
            # Validate that each ingredient's unit is compatible with its product's unit
            errors = []
            domain_recipe.ingredients.each do |ingredient|
              product = products[ingredient.product_id]
              begin
                ingredient.quantity.assert_compatible_unit!(product.unit)
              rescue Domains::Recipe::InvalidQuantityError
                errors << "Invalid unit '#{ingredient.quantity.unit}' for product '#{product.name}' (expected: '#{product.unit}')"
              end
            end
            # Raise if any unit compatibility errors were found
            unless errors.empty?
